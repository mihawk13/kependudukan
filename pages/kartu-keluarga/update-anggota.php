...
$query_cek_anggota="SELECT COUNT(*) AS jumlah from warga_has_kartu_keluarga where id_pdd=$id_pdd";
	...
	if ($cek_nik>0){
		...
		*/
		...
		echo "<script>window.alert('Warga sudah terdaftar di Kartu Keluarga lain !'); window.history.back()</script>";
				...
				$query_cek = "SELECT COUNT(*) AS total FROM warga_has_kartu_keluarga WHERE id_pdd = $id_pdd AND id_keluarga = $id_keluarga";
		...
		//echo $jumlah_cek;
				...
				//if ($jumlah_cek > 0) {
					...
					$query = "INSERT INTO warga_has_kartu_keluarga (id_pdd, id_keluarga) VALUES ('$id_pdd', '$id_keluarga');";
					//echo $query;
